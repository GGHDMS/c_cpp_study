...
void input(Queue *qPtr)
{
	...
	Person np;
	while (1) {
		printf("enqueue할 이름을 입력하시오(end 입력 시 종료) : ");
		scanf("%s", tname);
		if (strcmp("end", tname) == 0) { /* 문자나 EOF가 입력되면 while문을 빠져 나감*/
			break;
		}
		np.name = (char *)calloc(strlen(tname) + 1, sizeof(char));
		strcpy(np.name, tname);
		printf("enqueue 할 나이를 입력하시오 : ");
		scanf("%d", &np.age);
		printf("enqueue 할 전화번호를 입력하시오 :");
...
void myDelete(Queue *qPtr)
{
	...
	Person np = {NULL, 0, ""}; /* dequeue한 데이터를 저장할 변수*/
