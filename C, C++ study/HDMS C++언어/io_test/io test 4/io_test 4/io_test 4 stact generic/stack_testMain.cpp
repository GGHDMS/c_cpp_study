...
int main()
{
	...
	if (createStack(&stk, 5, sizeof(Person)) == FALSE) { /* 스택 초기화*/
		return 1;
	}
...
void mInput(Stack *sp)
{
	...
	while (1) {
		printf("Push할 이름을 입력하시오(end 입력 시 종료) : ");
		scanf("%s", tname);
		if (strcmp(tname, "end") == 0) { /* end 입력되면 while문을 빠져나감*/
			break;
		}
...
void myDelete(Stack *sp)
{
	...
	for (i = 0; i<cnt; i++) {
		...
		if (res == TRUE)  /* 성공적으로 pop 작업을수행했으면*/
		{
			personPrint(&np);
		}
		else
