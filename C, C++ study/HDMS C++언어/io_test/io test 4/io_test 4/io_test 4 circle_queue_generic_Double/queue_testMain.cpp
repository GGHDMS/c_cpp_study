...
void input(Queue *qPtr)
{
	...
	int res;
	while (1) {
		printf("enqueue할 실수를 입력하시오(문자 입력 시 종료) : ");
		res = scanf("%lf", &num);
		if (res != 1) { /* 문자나 EOF가 입력되면 while문을 빠져 나감*/
			myFlush();
			break;
		}
		if (!enqueue(qPtr, &num, sizeof(double), doubleMemcpy))
			printf("enqueue 실패!!\n");
	}
}
/*------------------------------------------------------------------------------------------------------------
...
void myDelete(Queue *qPtr)
{
	...
	double num; /* dequeue한 데이터를 저장할 변수*/
	int res; /* dequeue()함수의 리턴값을 저장할 변수*/
	printf("Queue에서 데이터를 dequeue할 회수를 입력하시오: ");
	scanf("%d", &cnt);
	for (i = 0; i<cnt; i++) {
		res = dequeue(qPtr, &num, sizeof(double), doubleMemcpy, doubleClear);
