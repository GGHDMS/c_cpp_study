...
int main()
{
	...
	destroyList(&list, doubleClear);	/* 리스트내의 모든 데이터 삭제 */
...
void mInput(List *lp)
{
	...
	while (1) {
		printf("# 실수값을 입력하세요(문자 입력시 종료) : ");
		res = scanf("%lf", &inData);	/* scanf()함수의 리턴 값 : 정수 입력 시 1, 문자 입력 시 0리턴 됨*/
		if (res == 0) {	/* 문자 입력 시 종료 */
			myflush();
			break;
		}
		bres = addLast(lp, &inData, sizeof(double), doubleMemcpy);	/* tail 노드 앞에 데이터 추가  */
		if (bres == TRUE)
			printf("@ 데이터 추가 성공!\n");
		else
			printf("@ 데이터 추가 실패!\n");
